...
// Хелпер: возвращает лучшее отображаемое имя пользователя
function displayName(user: { firstName?: string | null; username?: string | null; id: number }): string {
...
export class GroupsService {
  ...
  async getGroupById(groupId: string) {
    ...
    return {
      ...
      members: group.members.map((m) => ({
        ...
        firstName: m.user.firstName, // ← реальный firstName
        userId: m.user.id,
        user: {
          firstName: m.user.firstName, // ← реальный firstName
          username: m.user.username,
  ...
  async getInvitationsForUser(userId: number) {
    ...
    return invitations.map((inv) => ({
      ...
      // Используем displayName для корректного отображения имени пригласившего
      invitedByName: displayName(inv.invitedBy),
